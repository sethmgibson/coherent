...
2. Run `coherent doctor`, then `coherent audit` and `coherent plan`.
3. Create a baseline only when it is missing, stale, or the user intentionally
...
1. Treat `audit` as raw mechanical signal. The reviewed cleanup DAG from
   `plan` is the action queue.
...
4. Run `coherent fix next`, perform only that node, then typecheck, run targeted
   tests, audit, check, and plan again.
...
repository also requires `pnpm check:skill-docs` and
`pnpm check:unique-fingerprints`.
