...
**10**. Before a full cleanup, run the exact CLI that will perform the
work and require every capability used by this playbook:

```bash
coherent version . --json --expect-workflow 2 --expect-detector 10 --require-capability canonical-skill-path doctor-ref doctor-staged inspect-terminal-state review-aware-check review-queue self-describing-json python-ast
```
...
The required capabilities are: `canonical-skill-path`, `doctor-ref`, `doctor-staged`, `inspect-terminal-state`, `review-aware-check`, `review-queue`, `self-describing-json`, `python-ast`.
