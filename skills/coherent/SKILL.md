...
`audit`, `inspect`, `plan`, `check`, `doctor`, and `fix next` do not write Coherent metadata by default. `audit` and `plan` may write JSON only when the user supplies `--output <path>`. Do not create alternative reports, TODO files, scratch scripts, skills, rules, hooks, or planning documents. Optional Cursor/Git integrations still require explicit `--adapter`, `--alias`, `--rule`, `--cursor-hook`, or `--git-hook`. Skill-tree adoption uses `coherent install --yes` or `--providers` / `--scope`.
